...
const Multimessages=()=>{
  ...
  useEffect(() => {
    axios.get('http://localhost:8000/api/mail/list/marvin@telvoip.io/messages/')
      .then((response) => {
        setMessages(response.data.messages);
        ...
        const detailPromises = mailIds.map((element) => {
          return axios.get(`http://localhost:8000/api/mail/list/marvin@telvoip.io/messages/${element.id}`)
            .then((res) => {
